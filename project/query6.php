...
<?php	
$sql="SELECT name, MAX(s.cnt), MIN(s.cnt) FROM
(SELECT T.Name AS name, count(A.AccID) AS cnt
FROM accomodation A, touristplace T WHERE
T.AccID = A.AccID GROUP BY A.AccID) s;
";
...
<tr><td><?php echo $row["name"]; ?></td>
	<td><?php echo $row["MAX(s.cnt)"]; ?></td>
	<td><?php echo $row["MIN(s.cnt)"]; ?></td>
